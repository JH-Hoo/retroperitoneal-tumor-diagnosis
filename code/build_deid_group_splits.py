...
SOURCE_CSV = PROJECT_ROOT / "data" / "labels_5class_holdout" / "all.csv"
PRIVATE_ROOT = PROJECT_ROOT / "data_private"
SALT_PATH = PRIVATE_ROOT / "deid_salt.txt"
OUT_ROOT = PROJECT_ROOT / "data" / "labels_5class_groupcv_deid"
AUDIT_ROOT = PROJECT_ROOT / "data" / "audits_deid"
...
def main():
        ...
        private_rows.append(
            {
                ...
                "legacy_split_5class": r["split_5class"],
                "fold": str(fold_by_index[i]),
                "label_5": r["label_5"],
            }
        )

    write_rows(OUT_ROOT / "all_deid.csv", rows)
    ...
    for patient_hash, patient_rows in by_patient.items():
        ...
        legacy_splits = sorted(
            set(pr["legacy_split_5class"] for pr in private_rows if pr["patient_uid_hash"] == patient_hash)
        )
    ...
    if duplicate_rows:
        write_rows(AUDIT_ROOT / "duplicate_patient_audit_deid.csv", duplicate_rows)
    if legacy_leak_rows:
        write_rows(AUDIT_ROOT / "legacy_split_leakage_audit_deid.csv", legacy_leak_rows)
    ...
    write_rows(AUDIT_ROOT / "fold_label_counts.csv", count_rows)
    ...
    summary = {
        "name": "labels_5class_groupcv_deid",
        "source": "data/labels_5class_holdout/all.csv with PHI stripped",
        "num_cases": len(rows),
        ...
        "phi_policy": "GitHub keeps only deidentified CSV/JSON; salt and PHI audit stay in data_private/ and are gitignored.",
    }
