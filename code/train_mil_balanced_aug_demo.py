...
DATASET_ROOT = PROJECT_ROOT / "dataset_96slice_balanced_aug_v0"
CSV_PATH = DATASET_ROOT / "all.csv"
OUT_DIR = PROJECT_ROOT / "experiments" / "mil_resnet18_balanced_aug_96slice_10ep"
...
def load_cached_bag(tensor_path):
    x = torch.load(tensor_path, map_location="cpu").float().div(255.0)
...
class CTBags(Dataset):
    ...
    def __getitem__(self, i):
        r = self.rows[i]
        x = load_cached_bag(DATASET_ROOT / r["tensor"])
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"epoch {epoch}/{EPOCHS} train_loss={loss:.4f} train_acc={acc:.3f}", flush=True)
    ...
    print(json.dumps({k: metrics[k] for k in ["accuracy", "macro_f1", "weighted_f1", "confusion_matrix"]}, ensure_ascii=False, indent=2), flush=True)
